...
export const LocationsSection: React.FC<LocationsSectionProps> = ({ onOpenOrder }) => {
  ...
  // Proximity cursor interaction on each letter
  useEffect(() => {
  ...
  return (
    <section
      id="locations"
      ref={sectionRef}
      className="relative min-h-screen w-full bg-[#0A0A0A] text-[#F6EEE1] py-28 px-6 sm:px-12 flex flex-col items-center justify-center select-none overflow-hidden"
    >
      {/* Oxblood Radial Glow */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#4A0A10]/50 rounded-full blur-[140px] pointer-events-none"
        aria-hidden="true"
      />

      <div className="relative z-10 max-w-7xl mx-auto w-full text-center space-y-16">
        {/* Proximity Reactive Headline */}
        <div>
          ...
          <h2 className="font-display text-[clamp(4.5rem,14vw,12rem)] tracking-tighter uppercase leading-none select-none flex justify-center flex-wrap gap-x-3">
            {headlineText.split('').map((char, index) => (
              <span
                ...
                className="inline-block transition-transform will-change-transform text-[#F6EEE1] cursor-default"
              >
                {char === ' ' ? '\u00A0' : char}
              </span>
            ))}
          </h2>
          <p className="text-base sm:text-lg text-[#8C857C] font-body max-w-lg mx-auto mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {serviceChannels.map((card, idx) => {
            ...
            return (
              <div
                key={idx}
                className="relative bg-[#161413]/90 rounded-3xl p-8 border border-[#F6EEE1]/10 hover:border-[#E01B24]/60 transition-all duration-300 shadow-2xl flex flex-col justify-between space-y-6 group"
              >
                <div className="space-y-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#E01B24]/10 border border-[#E01B24]/30 flex items-center justify-center text-[#E01B24] group-hover:scale-110 group-hover:bg-[#E01B24] group-hover:text-[#F6EEE1] transition-all duration-300">
                    <Icon className="w-7 h-7" />
                  </div>
                  <h3 className="font-display text-3xl uppercase tracking-tight text-[#F6EEE1]">
                    {card.title}
                  </h3>
                  <p className="text-xs sm:text-sm text-[#8C857C] font-body leading-relaxed">
                    {card.line}
                  </p>
                </div>

                {/* Details list */}
                <div className="space-y-2.5 pt-4 border-t border-[#F6EEE1]/10 text-xs font-body text-[#8C857C]">
                  <div className="flex items-center gap-2 text-[#F6EEE1]">
                    <Clock className="w-3.5 h-3.5 text-[#F2B441] shrink-0" />
